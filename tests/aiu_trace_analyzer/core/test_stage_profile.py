...
def test_fwd_find(default_stage_checker):
    test_stages = [
        ('create_slice_from_BE', True, 2),
        ('do_not_move_index', False, 2),
        ('pipeline_barrier', True, 5),
    ]
...
def test_sparse_profile_supports_ordinal_for_repeated_stage_names(tmp_path):
    ...
    enabled_sequences = [index for index, (name, enabled) in enumerate(stage_profile.profile)
                         if name == "pipeline_barrier" and enabled]

    assert len(enabled_sequences) == 1
    enabled_index = enabled_sequences[0]
